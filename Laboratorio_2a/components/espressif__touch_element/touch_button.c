...
static volatile bool usado = false;
...
esp_err_t touch_button_install(const touch_button_global_config_t *global_config)
{
    ...
    TE_CHECK_GOTO(s_te_btn_obj->global_config != NULL, cleanup);
...
void touch_button_uninstall(void)
{
    ...
    if (usado == false) {
        ...
        while (!SLIST_EMPTY(&s_te_btn_obj->handle_list)) {
            te_button_handle_list_t *item = SLIST_FIRST(&s_te_btn_obj->handle_list);
...
esp_err_t touch_button_create(const touch_button_config_t *button_config, touch_button_handle_t *button_handle)
{
    ...
    ret = te_dev_init(&te_button->device,
                      1,
                      TOUCH_ELEM_TYPE_BUTTON,
                      &button_config->channel_num,
                      &button_config->channel_sens,
                      TE_DEFAULT_THRESHOLD_DIVIDER(s_te_btn_obj));
    ...
    *button_handle = (touch_elem_handle_t)te_button;
    ...
    ESP_LOGD(TE_DEBUG_TAG,
             "channel: %d, channel_sens: %f",
             button_config->channel_num,
             button_config->channel_sens);
                                           ...
                                           touch_elem_dispatch_t dispatch_method)
{
    ...
    TE_CHECK(dispatch_method >= TOUCH_ELEM_DISP_EVENT &&
             dispatch_method <= TOUCH_ELEM_DISP_MAX,
             ESP_ERR_INVALID_ARG);
...
esp_err_t touch_button_subscribe_event(touch_button_handle_t button_handle,
                                       uint32_t event_mask,
                                       void *arg)
{
    TE_CHECK(s_te_btn_obj != NULL, ESP_ERR_INVALID_STATE);
    ...
    if (!(event_mask & TOUCH_ELEM_EVENT_ON_PRESS) &&
        !(event_mask & TOUCH_ELEM_EVENT_ON_RELEASE) &&
        !(event_mask & TOUCH_ELEM_EVENT_ON_LONGPRESS) &&
        !(event_mask & TOUCH_ELEM_EVENT_NONE)) {
        ESP_LOGE(TE_TAG,
                 "Touch button only support TOUCH_ELEM_EVENT_ON_PRESS, "
...
esp_err_t touch_button_set_longpress(touch_button_handle_t button_handle,
...
static void button_object_update_state(touch_pad_t channel_num,
                                       te_state_t channel_state)
{
    te_button_handle_list_t *item;
    ...
    if (usado == false) {
        ...
        SLIST_FOREACH(item, &s_te_btn_obj->handle_list, next) {
            if (button_handle == item->button_handle) {
                SLIST_REMOVE(&s_te_btn_obj->handle_list,
                             item,
                             te_button_handle_list,
                             next);
...
static bool button_channel_check(te_button_handle_t button_handle,
...
static void button_update_state(te_button_handle_t button_handle,
                                touch_pad_t channel_num,
                                te_state_t channel_state)
{
...
static inline void button_dispatch(te_button_handle_t button_handle,
                                   touch_elem_dispatch_t dispatch_method)
{
    if (dispatch_method == TOUCH_ELEM_DISP_EVENT) {
        ...
        button_handle->config->callback(button_handle,
                                        &button_info,
                                        button_handle->config->arg);  //Event callback
    }
}

void button_enable_wakeup_calibration(te_button_handle_t button_handle,
                                      bool en)
{
